  ...
  void markAttendance() async {
    try {
      ...
      DocumentSnapshot data =
          await db.collection('Attendance').document(widget.students[0]).get();
      int length = data[widget.course]['Attendance'].length;
      if (data[widget.course]['Attendance'][length - 1]['date'].toString() ==
          formattedDate) {
        setState(() {
          _load = false;
        });
        ...
        for (var i = 0; i < widget.students.length; i++) {
          ...
          await batch.updateData(
              ...
              {'${widget.course}.Attendance': FieldValue.arrayUnion(obj)});
        }
        await batch.commit();
        setState(() {
          _load = false;
